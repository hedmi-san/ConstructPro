...
public class RestSalairePayment extends JDialog {
    private static final Color DARK_BACKGROUND = new Color(45, 45, 45);
    private PaymentCheckDAO paymentCheckDAO;
    private SalaryRecordDAO salaryRecordDAO;
    private Worker worker;
    private JLabel titleLabel;
    private JTextField restField, paidAmountField;
    private JButton saveButton;

    public RestSalairePayment(JFrame parent, Worker worker, Connection connection) throws SQLException {
        super(parent, "Payer le Reste", true);
    ...
    private void initComponents(String workerFullName) {
        ...
        // Text fields
        restField = createTextField();
        restField.setEditable(false); // Read-only field
    ...
    private void setupLayout() {
        ...
        // Rest amount (read-only)
        gbc.gridx = 0;
        gbc.gridy = row;
        formPanel.add(createLabel("Reste à payer"), gbc);
        gbc.gridx = 1;
        formPanel.add(restField, gbc);
        row++;
    ...
    private void setupActions(int workerId) {
        try {
            // Load and display the remaining amount
            SalaryRecord record = salaryRecordDAO.getOrCreateSalaryRecord(workerId);
            double remaining = record.getTotalEarned() - record.getAmountPaid();
            restField.setText(String.format("%.2f", remaining));
        } catch (SQLException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Erreur lors du chargement du reste : " + ex.getMessage());
        }
        ...
        saveButton.addActionListener(e -> {
            try {
                ...
                // Validate that paid amount doesn't exceed remaining amount
                if (paidAmt > remaining) {
                    JOptionPane.showMessageDialog(this,
                            "Le montant ne peut pas être supérieur au reste (" + String.format("%.2f", remaining)
                                    + ") !",
                            "Montant invalide",
                ...
                // Create payment check with baseSalary = 0
                LocalDate paymentDate = LocalDate.now();
                paymentCheckDAO.insertPaymentCheck(
                        record.getId(),
                        worker.getAssignedSiteID(),
                        paymentDate,
                        0.0, // baseSalary is 0 for rest payment
                        paidAmt);

                // Update salary record totals
                salaryRecordDAO.updateSalaryRecordTotals(record.getId());

                JOptionPane.showMessageDialog(this, "Paiement du reste enregistré avec succès !");
                dispose();
